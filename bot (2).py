...
def check_new_trades(s):
    ...
    for t in s["traders"]:
        ...
        if CFG["sell_when_they_sell"]:
            ...
            for x in new:
                if x.get("side") == "SELL":
                    ...
                    sell_cost[a] = sell_cost.get(a, 0.0) + sz * float(x.get("price", 0))
        ...
        for asset, g in buys.items():
            ...
            if entry < their_px:
                # we can't realistically get in cheaper than they did after a delay;
                # a lower number means the price feed was stale, so be conservative
                entry = their_px
...
def maybe_reset(s):
    ...
    if s.get("reset_id", 0) != rid:
        s["positions"], s["skipped"], s["reset_id"] = [], {}, rid
